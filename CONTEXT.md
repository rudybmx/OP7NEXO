...
### 🟡 Parcial (2026-06-24) — Central de Agentes: Fase 2 (núcleo do agente; SEM o hook de fluxo vivo)
...
- **PENDENTE (próximo incremento — mexe no fluxo de produção):** hook de inbound com debounce (`whatsapp_crm_persistence.process_evolution_message` ~l.407, enqueue `crm_message_jobs` job_type=`agente_reply`, UPDATE com guarda `status='pending'`), branch no `whatsapp_event_worker`, `processar_reply` (envio outbound **por provider** — sem helper unificado hoje em `canais.py`), seed de preço em `ai_model_pricing`.
