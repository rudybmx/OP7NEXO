...
- **PENDENTE Fase 2.x:** envio outbound p/ **waha/meta/instagram** (hoje só Evolution; demais providers → handoff seguro); seed de preço em `ai_model_pricing` (sem isso, custo no dashboard fica 'sem_preco'); persistência explícita da msg de saída (hoje vem pelo echo fromMe do provider). **Deploy do worker exige `deploy.sh worker`** (não entra no `both`).
