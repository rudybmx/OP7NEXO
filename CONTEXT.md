...
> max_tentativas/intervalo_horas/hora_inicio-fim) [migration **112**]. Worker
...
### ✅ Implementado (2026-06-26) — Agente: seleção de agenda por agente (Fase 6, multi-clínica)
- O agente de IA agora pode ser **restrito a agendas específicas** (multi-clínica: cada clínica = canal = agente). Migration **109** `agente_agendas` (junção M:N `agente_id`×`agenda_id`, unique, FK CASCADE) + model `app/models/agente/agente_agenda.py` (relationship `Agente.agendas`). **Escopo num ponto único:** `agente_tools._agendas_agendaveis(db, workspace_id, agenda_ids=None)` ganhou o filtro `Agenda.id.in_(agenda_ids)`; threado por `tools_para_workspace`/`_resolver_agenda`/`_exigir_agenda`/`_agendamentos_futuros_do_contato`/`executar_tool` e os 6 `_tool_*`. `agent_service.gerar_resposta` calcula `agenda_ids = {l.agenda_id for l in agente.agendas} or None` e passa adiante. **Fallback:** agente SEM vínculo → `agenda_ids=None` → atende TODAS as agendáveis do workspace (zero regressão p/ clínica única). Casa o profissional **pelo NOME da agenda** (match exato/contém já existente — "Dr. Gumercindo"). CRUD `app/api/agentes.py`: `_validate_agendas`/`_apply_agendas`/`_agendas_out` (espelham canais) + campo `agendas: [agenda_ids]` em `AgenteIn`/`AgenteUpdate`/`AgenteOut`. Front: multiselect "Agendas que o agente pode marcar" em `agente-form.tsx` (vazio=todas). Testado: boot-import + migration 109 up/down/up + teste de escopo (2 agendas, agente vinculado a 1 → só ela no prompt/resolução; "Silva" da outra clínica não vaza; sem vínculo → as 2).

### ✅ Implementado (2026-06-26) — Central de Agentes: modo "plantão" + fuso no horário
- Campo `agentes.horario_modo` (migration **104**, default `'dentro'`, CHECK in `('dentro','fora')`). Controla como as janelas de `agente_horarios` são lidas em `agent_service.dentro_do_horario`: `'dentro'` = responde NAS janelas (comportamento atual); `'fora'` (plantão) = responde FORA delas — cobre noites + fins de semana automaticamente (o usuário cadastra só o horário comercial e marca "responder fora"). Sem janelas = 24/7 nos dois modos.
- **Fix de fuso**: `dentro_do_horario` passou a usar **horário de Brasília** (UTC-3 fixo, igual `_contexto_temporal`) no default de `agora`, em vez de UTC — as janelas são cadastradas/interpretadas em Brasília. Pré-flight: `agente_horarios` estava **vazia** em todo o banco → zero regressão. Roda no **worker** (`processar_reply`), NÃO em `/testar` (testar a lógica direto: função pura). Schema/CRUD (`AgenteIn`/`Update`/`Out` + `criar`/`atualizar`/`_agente_out`) expõem `horario_modo`. Front: toggle "responde dentro/fora do horário" na seção Horários + aviso (Brasília; sem horário = 24/7).
### ✅ Implementado (2026-06-26) — Agenda: auto-agendamento público (Fase 5)
- Link público **por agenda** p/ o paciente marcar sozinho, sem login. Migration **106** `agenda_public_tokens` (token CSPRNG 256-bit, `agenda_id` FK CASCADE, `workspace_id`, status active|revoked; índice parcial único `WHERE status='active'` = 1 token ativo/agenda; **LONGEVO e reusável** — não consome por reserva, diferente do `canal_connect_tokens`). Serviço `app/services/agenda/public_token.py` (gerar_ou_reusar atômico, buscar_token_valido, revogar). Router público **SEM auth** `app/api/public_agendar.py` (prefixo `/public/agendar`, registrado em main): `GET /{token}` (agenda+clínica+serviços+pode_agendar), `GET /{token}/disponibilidade?data&servico_id` (slots), `POST /{token}` (cria, `origem='paciente'`). Admin: `POST/DELETE /agenda/agendas/{id}/link-publico` (gera/revoga, `_agenda_autorizada`). **Guards de segurança (público):** todo id vem do TOKEN nunca do corpo; o POST **re-valida o slot** via `calcular_disponibilidade` (recusa 409 fora-de-horário/almoço/bloqueio/passado/ocupado — `criar_agendamento` é permissivo e não basta); serviço tem que ser da agenda ou global (senão 422); **rate-limit por IP (10/h) + telefone (5/h) fail-closed** (NÃO por token — DoS de 1 link travaria a clínica), GET fail-open. Autonomia por `agente_agendamento`: `desativado`→403/sem link, `direto`→`status='confirmado'`, `confirmar`→`status='agendado'` (fila, filtrável por status+origem) + observação. Spec `docs/specs/agenda-publico/`. Testado: TestClient contra **schema real de prod** em DB scratch (8 guards verdes: book/origem, 03h→409, passado→409, serviço estrangeiro→422, token→404, direto→confirmado, confirmar→agendado) + migration 106 up/down + boot-import.

### ✅ Implementado (2026-06-26) — Agenda: lembretes por WhatsApp (Fase 4)
- Lembrete automático por WhatsApp antes do agendamento (ex.: 1 dia antes, 09:00). Migration **105** (`agenda_lembrete_config` + `agenda_lembrete_envios` = log de dedupe durável; renumerada de 104→105 por colisão com `agente_horario_modo`). Serviço `app/services/agenda/lembretes.py`: `processar_lembretes_pendentes()` é uma **varredura** (job `interval` 5min em `scheduler.py`, padrão dos followups) — acha agendamentos futuros ocupantes no escopo da config (agenda específica OU global `agenda_id NULL`), calcula `horario_envio` (tz da agenda), e p/ os vencidos **ainda-não-enviados** (sem linha no log) renderiza o template (`{{nome}}/{{data}}/{{hora}}/{{servico}}/{{profissional}}`), acha a **conversa+canal do contato** (por `remote_jid LIKE telefone_normalizado@%`; **só contatos com conversa** = opt-in), envia via `agent_service._enviar_resposta`, persiste a saída (`remetente_tipo='sistema'`) e **loga** (dedupe por agendamento×config → não re-spamma). CRUD `/agenda/lembretes` (GET/POST/PATCH/DELETE, padrão dos serviços). **Lembrete padrão** (1 dia antes, 09:00) criado no POST de agenda (best-effort, nunca derruba a criação). **Resposta do paciente** = tratada pelo agente (Fase 3) + nova tool **`confirmar_presenca`** (marca `confirmado`). v1: **só WhatsApp** (email/sms/push do mockup ficam inertes no backend); **NPS = fase separada**. Spec `docs/specs/agenda-lembretes/`. ⚠️ Front: religar `use-lembretes` (mock→API) PENDENTE. Testado: migration up/down/up isolado + boot-import + tz/template puros (scan completo validado pós-deploy).

### ✅ Implementado (2026-06-26) — Agenda: agente de IA agenda sozinho (tool-calling, Fase 3)
- O agente de WhatsApp ganhou **tool-calling** (loop agêntico) p/ consultar/marcar/remarcar/cancelar agendamentos dentro da conversa. NOVO `app/services/agenda/agente_tools.py`: 5 tools (`consultar_disponibilidade`, `buscar_agendamentos_contato`, `criar_agendamento`, `reagendar_agendamento`, `cancelar_agendamento`) chamando os serviços de domínio da Agenda (Fase 1); o **executor injeta `workspace_id` + telefone da conversa** (de `remote_jid`) — o modelo só passa data/serviço/nome (multi-tenant + vínculo por telefone). `tools_para_workspace()` só oferece as tools se houver agenda com `agente_agendamento != 'desativado'` (**zero regressão** sem agenda). Autonomia: `desativado`=sem tools; `direto`=marca; `confirmar`=confirma o horário com o cliente antes de gravar (diferença só de prompt — decisão do usuário, sem aprovação de equipe). `llm_client_service.chamar_com_tools()` (mantém `response_format=json_object` + `tools`); o loop vive em `agent_service.gerar_resposta()` (cap 5 rodadas + fallback; `telefone_contato` vem de `processar_reply` via `remote_jid`). Erros de domínio → `{erro}` (não quebram o loop); falha de LLM → handoff (comportamento atual). **★Decisão-chave: com tools ativas, `gerar_resposta` usa `modo_tools` → NÃO força `response_format=json_object` e troca a instrução "responda em JSON" por "chame a tool e responda em TEXTO" (`chamar_com_tools(forcar_json=False)`); a resposta final é texto natural (score 0.9, intent "agenda").** Sem isso, modelos menores (deepseek) geram a resposta-JSON em vez de chamar a ferramenta (obedecem a instrução de JSON). Validado ao vivo: **deepseek-v4-flash (via opencode, o modelo do agente real) E gpt-4o-mini ambos agendam** (consultam horários reais → criam). Sem tools, o caminho JSON antigo é intacto (zero regressão; nome_cliente só é capturado nesse caminho). Testado: executor contra DB real (consultar/criar/anti-double-book/buscar/reagendar/cancelar) + boot-import + tz local→UTC. Spec: `docs/specs/agenda-agente-tools/`.
...
### ✅ Corrigido (2026-06-26) — Gerador de criativos cortava a arte (regressão do criativos-2)
- O reframe `criativo_render.ajustar_para_canvas` (object-cover → `_cover`, recorta a base 2:3 do gpt-image pro EXPORT_SIZE do canal) era aplicado em `_compose_e_persistir`, **compartilhado** pelo gerador (`/design/gerar`) e pelo carrossel (`carrossel_gen`). Introduzido na onda do criativos-2 (commits 19-23/jun), passou a **cortar bordas e comer o respiro** do criativo do gerador original (sintoma reportado no arquivo final).
- Fix: parâmetro `recortar` (default **True** → carrossel e multiformato inalterados) em `ajustar_para_canvas`/`_compose_e_persistir`/`executar_geracao_integrada`/`executar_geracao_multiformato`. A rota `/design/gerar` passa `recortar=False` **só no caminho de 1 formato** (uso padrão) → arte INTEIRA no tamanho nativo (retrato sai 2:3), sem corte. Multiformato (opt-in de N aspectos) e carrossel seguem recortando. Sem migration; `EXPORT_SIZES` intactos.

### ✅ Implementado (2026-06-26) — Nome confirmado do contato (anti-push_name) [Fase 1]
- **Migration 102** (down_revision=101): colunas `crm_whatsapp_contatos.nome_confirmado` + `nome_origem` (`'humano'`|`'ia'`|NULL). Abordagem ADITIVA — `nome_confirmado` = nome declarado pelo cliente / editado por humano, separado do `push_name` (nome do WhatsApp, não-confiável). Backfill NÃO-destrutivo: popula `nome_confirmado` dos nomes legados que parecem reais; NUNCA altera `nome`/`push_name`.
- **NÃO toca caminhos de escrita de inbound** (webhooks Evolution/WAHA/Meta `_processar_mensagem_*`, enriquecimentos) — decisão consciente p/ não editar handlers de produção (auditoria achou 4 pontos copiando push→nome). `nome` segue sendo o campo legado/fallback.
- **Display `nome_confirmado > push_name > nome`**: `conversas._resolved_contact_name` + retorno de iniciar-conversa.
- **Agente** (`agent_service._montar_system`/`processar_reply`): usa só `nome_confirmado`; sem ele, regra global "NUNCA presuma o nome (o do WhatsApp é não-confiável), pergunte".
- **Contatos** (`POST`/`PUT /contatos`): criação/edição manual grava `nome_confirmado` + `nome_origem='humano'`; `ContatoOut` expõe ambos.
- **[Fase 2 — feita]** IA grava `nome_confirmado` (origem `'ia'`) quando o cliente DECLARA o próprio nome — 2 caminhos: agente em conversa (campo `nome_cliente` no structured output → `processar_reply`) e análise por conversa (`_ANALISE_PROMPT_VERSAO`=2 → `processar_analise`). Helpers `_validar_nome_cliente`/`_aplicar_nome_confirmado_ia` (guarda: NUNCA sobrescreve `nome_origem='humano'` + idempotência).
- **[Fase 3 — feita]** `processar_analise` espelha a ÚLTIMA análise no cadastro do contato (last-write): `resumo_ia` + `sentimento_ia` (temperatura) + `score_lead_ia` em colunas próprias; `interesse`/`observacoes`/`temperatura` em `perfil_json.contexto_ia` (`jsonb_set`, preserva o resto). Sem migration (campos já no `ContatoOut`).
- **[Fase 4 — feita]** RBAC da tela de contatos. `crm_escopo`: `aplicar_teto_contatos` (company_agent vê só os DELE — responsável do contato OU teve conversa; supervisores veem todos), `pode_ver_contato`, `pode_acessar_tela_contatos`. `contatos.py`: gate em TODO o router (`exigir_acesso_contatos`→403; supervisor sempre, company_agent só com `pode_acessar_crm`) + teto na listagem e nos by-id (404 p/ não vazar). ⚠️ `pode_acessar_crm` default false → habilitar p/ os atendentes certos antes/depois do deploy.
- Roadmap: ~~Fases 1–5 + Fase 4 (backend)~~ feitas · falta: ocultar menu "Contatos" no front p/ quem não tem acesso (gate visual) + DEPLOY (api+worker+front).
...
- Testado: 12 testes puros (telefone+disponibilidade) + migration up/down + EXCLUDE + serviço (slot/capacidade/variante 9dig/terceiro) em DB scratch. Front (religar mockup `use-agendas`/`use-agendamentos` + sidebar grupo AGENDA) deployado+verificado (Playwright).
- **Fase 1.5 (catálogo de serviços)**: migration 102 — `agenda_servicos` (por agenda; `agenda_id` NULL = serviço do workspace; `duracao_minutos` guia o slot, `preco`) + `agendamentos.servico_id` FK (mantém `servico` text como snapshot). CRUD `/agenda/servicos` (GET filtra workspace + agenda_id incluindo NULL); `GET /agenda/disponibilidade?servico_id=` resolve a duração; `criar_agendamento(servico_id=)` snapshota o nome. Validado em scratch (migration up/down + snapshot).
