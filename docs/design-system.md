...
- **Fase 1 (feita):** HeroUI removido — showcase `design-system-v2/` (27 arq.) deletado; 7 usos reais migrados (Button, Input→`<input>` nativo, Tabs, Switch, Chip→Badge, Table→`<table>`+`.ds-table`+`WSTableShell`); `react-icons`→`lucide` (7 arq.); 44 `@import @heroui` + deps removidos. `next build` OK; verificado visualmente com dados. **Débito conhecido:** a camada de tokens HeroUI-derivada (2º `@theme inline` + bloco `:root`) permanece (sai na F2); os `<input>` nativos viram `ui/input` na F3.
