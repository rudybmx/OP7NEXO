...
- **Fase 1 (feita):** HeroUI removido — showcase `design-system-v2/` (27 arq.) deletado; 7 usos reais migrados (Button, Input→`<input>` nativo, Tabs, Switch, Chip→Badge, Table→`<table>`+`.ds-table`+`WSTableShell`); `react-icons`→`lucide` (7 arq.); 44 `@import @heroui` + deps removidos. `next build` OK; verificado visualmente com dados. **Débito conhecido:** a camada de tokens HeroUI-derivada (2º `@theme inline` + bloco `:root`) permanece (sai na F2); os `<input>` nativos viram `ui/input` na F3; o `TabsList` (cliente-form) usa `bg-muted` = `#9ca3af` (cinza médio, funcional) — corrige sozinho quando `muted` for redefinido na F2.
- **Fase 2:** remover a camada de tokens HeroUI (2º `@theme inline` + `:root` `--default`/`--success`/`--danger`…) + `--ws-*`/`glass-card`/objetos de estilo de `src/lib/utils.ts` → shadcn; **redefinir `accent`/`muted`/`secondary`/`bg-popover`** (hoje presos ao legado); consolidar showcase único.
