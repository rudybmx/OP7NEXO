...
- **Render WYSIWYG**: template é layout JSON. Front renderiza em DOM (edição instantânea). Export final re-renderiza o **mesmo** template via **Playwright headless Chromium** num **job no `op7nexo-worker`** (tabela `criativo_export_jobs`, polling estilo `sync_jobs`). Fontes da marca provisionadas no container do worker e no front.
